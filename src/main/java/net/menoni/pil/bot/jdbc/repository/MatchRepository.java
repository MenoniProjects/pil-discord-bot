...
public class MatchRepository extends AbstractTypeRepository<JdbcMatch> {
	...
	public List<JdbcMatch> all() {
		return this.queryMany("SELECT id, division, round_number, first_team_id, second_team_id, match_channel_id, pin_message_id, win_team_id, win_team_score, lose_team_score FROM `match`");
	}
	
	public List<JdbcMatch> findMatchesForTeam(Long teamId) {
		return this.queryMany(
				"SELECT id, division, round_number, first_team_id, second_team_id, match_channel_id, pin_message_id, win_team_id, win_team_score, lose_team_score FROM `match` " +
	...
	public JdbcMatch save(JdbcMatch match) {
		if (match.getId() == null) {
			GeneratedKeyHolder key = this.insertOne(
					"INSERT INTO `match` (division, round_number, first_team_id, second_team_id, match_channel_id, pin_message_id, win_team_id, win_team_score, lose_team_score) VALUE " +
							"(:division, :roundNumber, :firstTeamId, :secondTeamId, :matchChannelId, :pinMessageId, :winTeamId, :winTeamScore, :loseTeamScore)",
					match
			);
			if (key != null) {
			...
			this.update("UPDATE `match` SET first_team_id = :firstTeamId, second_team_id = :secondTeamId, match_channel_id = :matchChannelId, pin_message_id = :pinMessageId, " +
					...
					.add("pinMessageId", match.getPinMessageId())
	...
	public JdbcMatch findByChannel(String channelId) {
		return this.queryOne(
				"SELECT id, division, round_number, first_team_id, second_team_id, match_channel_id, pin_message_id, win_team_id, win_team_score, lose_team_score FROM `match` WHERE match_channel_id = ?",
				channelId
		);
	}
	...
	public List<JdbcMatch> findAllForRound(int round) {
		return this.queryMany(
				"SELECT id, division, round_number, first_team_id, second_team_id, match_channel_id, pin_message_id, win_team_id, win_team_score, lose_team_score FROM `match` WHERE round_number = ?",
				round
		);
	}
}
